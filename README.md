...
- **本地上传**：在首页「上传新 Skill」里点「选择 SKILL.md」，选择本地的 `.md` 文件。
- **从链接导入**：在「来源 URL」中粘贴 `.md` 的链接（如 GitHub 上的 SKILL.md/README.md），点「从链接导入」即可拉取并加入列表，无需本地文件。GitHub 的 `.../blob/...` 会自动转为 raw 拉取。
- **多平台适配**：上传或导入时可选「来源」（Cursor / GitHub）和「来源 URL」；系统会优先用对应适配器解析元数据，再结合 Claude 分析补全。
...
- **执行中**：对话/执行时会出现「处理中…」与 loading 动画；执行失败的消息旁有「重试」可再次执行。
- **错误与重试**：接口或网络错误时底部会提示，可点「重试」或「关闭」；列表加载失败也可点「重试」重新拉取。
- **空状态**：暂无 skill 时列表区会提示「请先上传 SKILL.md 或从链接导入」。
